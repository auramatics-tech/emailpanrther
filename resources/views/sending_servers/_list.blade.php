    ...
    >
        @foreach ($items as $key => $item)
            <tr>
                ...
                <td>
                    <div class="single-stat-box pull-left ml-20">
                        @if($item->type == 'multi-smtp')
                            @php
                            $quota_count = 0;
                            $linked_servers = \Acelle\Model\SendingServer::where('multi_server_linked_with',$item->id)->get();
                            if(count($linked_servers)){
                                foreach($linked_servers as $ls){
                                    $quota_count += $ls->getCreditsUsed('send');
                                }
                            }
                            @endphp
                            <span class="text-muted"><strong>{{ number_with_delimiter($quota_count) }}</strong> {{ trans('messages.' . Acelle\Library\Tool::getPluralPrase('email', $item->getCreditsUsed('send')) . '_quota') }}</span>
                        @else
