			...
			<tr>
				...
				<td>
					...
					@if ($campaign->status != 'new' && isset($campaign->run_at))
						<span class="text-muted2 d-block">{{ trans('messages.run_at') }}: <span class="material-symbols-rounded">alarm</span>
				...
				@if ($campaign->status != 'new')
					<td class="stat-fix-size-sm">
						<div class="single-stat-box pull-left ml-20">
							<span class="no-margin text-primary stat-num">{{  number_to_percentage($campaign->readCache('DeliveredRate')) }}</span>
							...
							@php
								$linked_camps = json_decode($campaign->linked_campaigns);
								$deliveredCount = 0;
								$SubscriberCount = 0;
								if(count($linked_camps)){
									foreach($linked_camps as $camps){
										$camp = Acelle\Model\Campaign::find($camps);
										if(isset($camp->id)){
											$deliveredCount += $campaign->readCache('DeliveredCount', 0);
											$SubscriberCount += $campaign->readCache('SubscriberCount', 0);
										}
									}
								}
							@endphp
							<span class="text-semibold text-nowrap">{{ $campaign->isSending() ? number_with_delimiter($deliveredCount) : number_with_delimiter($deliveredCount) }} / {{ number_with_delimiter($SubscriberCount)  }}</span>
							<br />
						...
						@if (\Gate::allows('delete', $campaign) || \Gate::allows('pause', $campaign) || \Gate::allows('restart', $campaign))
							<div class="btn-group ms-1" role="group">
								...
								<ul class="dropdown-menu dropdown-menu-end" aria-labelledby="btnGroupDrop1">
									...
									target="_blank"
									class="dropdown-item"
									href="/campaign-template/{{ base64_encode($campaign->uid) }}">
									<span class="material-symbols-rounded me-2">copy_all</span>Public Template</a></li>
								</ul>
							</div>
						@endif
					</div>
