<div data-variant="{{ $step->variants[0]->id }}" data-id="{{ $step->id }}" id="main_step_{{ $step->step_number }}" class="card step_card mb-3 step_card_{{ $step->id }} @if($key == 0) active @endif" data-step="{{ $step->step_number }}">
    <div class="card-header bg-none d-flex align-items-center justify-content-between py-3">
        ...
        <div>
            ...
            <button data-id="{{ $step->id }}" id="delete_step_{{ $step->step_number }}" type="button" class="border-0 bg-none delete_steps" data-step="{{ $step->step_number }}" @if(count($campaign->steps) == 1) style="display:none;" @endif>
                <span class="material-symbols-rounded fs-5 fw-bold">delete</span>
            </button>
        </div>
    </div>
    <div class="card-body bg-none" id="card_body_{{ $step->step_number }}">
        @include('campaigns.template.variants')
    ...
    <div class="card-footer bg-none d-flex gap-3 align-items-center justify-content-center py-3" @if(count($campaign->steps) == 1 || count($campaign->steps) == ($key+1)) style="display:none !important" @endif id="footer_step_{{ $step->step_number }}">
        <span>wait for</span>
