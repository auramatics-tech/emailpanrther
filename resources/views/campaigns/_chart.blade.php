            ...
            @php
                ...
                if(count($linked_camps)){
                    foreach($linked_camps as $camps){
                        ...
                        if(isset($camp->id)){
                            $delivery_count += $campaign->deliveredCount();
                            $uniqueOpenCount += $campaign->uniqueOpenCount();
                            $UniqOpenCount += $campaign->readCache('UniqOpenCount');
                            $UniqOpenRate += $campaign->readCache('UniqOpenRate');
                        }
                    }
                }
            @endphp
            <span class="badge bg-secondaryx bg-color0 badge-big me-2">{{ number_to_percentage($UniqOpenRate) }}</span>
            {{ trans('messages.opened') }}
            <span class="fw-600">
                {{ trans('messages.open_uniq_per_total', [
                    'count' => $UniqOpenCount,
                    'total' => $uniqueOpenCount,
