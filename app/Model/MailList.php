...
class MailList extends Model
{
    ...
    public function export($progressCallback = null, $segment = null)
    {
        ...
        if (!is_null($progressCallback)) {
            $progressCallback($processed, $total, $failed, $message = 'エクスポートが進行中です...');
        }
        ...
        cursorIterate($query, $orderBy = 'subscribers.id', $pageSize, function ($subscribers, $page) use ($writer, $listFields, &$processed, &$total, &$failed, &$message, $pageSize, $progressCallback) {
            ...
            // Callback
            if (!is_null($progressCallback)) {
                $progressCallback($processed, $total, $failed = 0, $message = sprintf('エクスポートが進行中です, %s / %s records written', $processed, $total));
            }
        });

        if (!is_null($progressCallback)) {
            $progressCallback($processed, $total, $failed = 0, $message = sprintf('エクスポート完了, 処理済み：%s / %s', $processed, $total));
        }
    }
    ...
    public function send($message, $params = [])
    {
        ...
        $message->getHeaders()->addTextHeader('X-Msg-ID', StringHelper::generateMessageId(StringHelper::getDomainFromEmail($this->from_email)));
        ...
        try {
            ...
            each_batch($results, config('app.import_batch_size'), false, function ($batch) use ($map, &$processed, &$failed, $total, &$overQuotaAttempt, &$message, $progressCallback, $invalidRecordCallback) {
                ...
                $insertToSubscribersSql = strtr(
                    '
                    INSERT INTO %subscribers (uid, mail_list_id, email, status, subscription_type, tags, created_at, updated_at)
                    SELECT SUBSTRING(MD5(UUID()), 1, 13), %list_id, uniq.email, %status, %type, uniq.tags, NOW(), NOW()
                    FROM (
