...
class Customer extends Model
{
    ...
    public function assignPlan($plan)
    {
        if ($this->getNewOrActiveSubscription()) {
            throw new Exception("This customer already has a new or active subscription");
        }
    ...
    public function verifiedIdentitiesDroplistDomains($keyword = null)
    {
        $droplist = [];
        $topList = [];
        $bottomList = [];
        ...
        $multiDomains = \Acelle\Model\SendingServer::where(['multi_server' => 1, 'multi_server_linked_with' => 0])->pluck('name')->toArray();

        foreach ($multiDomains as $item) {
            // check if email
            if (extract_email($item) !== null) {
                $email = extract_email($item);
                if (strpos(strtolower($email), $keyword) === 0) {
    ...
    public function domainAssignment()
    {
        return $this->hasMany('Acelle\Model\DomainAssignment');
    }
