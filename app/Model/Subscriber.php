...
class Subscriber extends Model
{
     ...
     * @param MailList $list
     */
    public function SenttrackingLog($campaign)
    {
        $query = \Acelle\Model\TrackingLog::where('tracking_logs.subscriber_id', '=', $this->id);
        $query = $query->where('tracking_logs.campaign_id', '=', $campaign->id)
        ->where('tracking_logs.status', 'sent')->orderBy('created_at', 'desc')->count();
    ...
    {
        $query = \Acelle\Model\OpenLog::select('open_logs.*')->leftJoin('tracking_logs', 'tracking_logs.message_id', '=', 'open_logs.message_id')
    ...
    {
        $query = \Acelle\Model\ClickLog::select('click_logs.*')->leftJoin('tracking_logs', 'tracking_logs.message_id', '=', 'click_logs.message_id')
            ->where('tracking_logs.subscriber_id', '=', $this->id);
    ...
    public function trackingLogsCampaign($campaign = null)
    {
        $query = \Acelle\Model\TrackingLog::where('tracking_logs.subscriber_id', '=', $this->id);
    ...
    public function generateUnsubscribeUrl($messageId = null, $absoluteUrl = true, $campaign_id = '')
    {
