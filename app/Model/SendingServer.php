...
class SendingServer extends Model implements HasQuotaInterface
{
    ...
    public const TYPE_MULTI_SMTP = 'multi-smtp';
    ...
    public static $serverMapping = array(
        ...
        self::TYPE_MULTI_SMTP => 'SendingServerSmtp',
    ...
    public function sendTestEmail($params)
    {
        ...
        $message->setId($msgId);
        $message->getHeaders()->addTextHeader('X-Msg-ID', $msgId); // this header is required for SendGrid API sending server
    ...
    public function doDelete()
    {
        ...
        ])->delete(env('EE_BASE') . "/account/$this->uid");

        $this->logger()->info('EE account deleted  ' . $this->uid . '  ' . json_encode($response->json()));

        // check if linked with campaign and campaign is active
        $campaign_servers = \Acelle\Model\CampaignSendingServer::where('sending_server_id', $this->id)->get();
        if (count($campaign_servers)) {
            foreach ($campaign_servers as $camp_ser) {
                $campaign = \Acelle\Model\Campaign::find($camp_ser->campaign_id);
                if (isset($campaign->id) && $campaign->status == 'paused') {
                    $camp_ser->delete();
                }else{
                    $camp_ser->delete();
                }
        ...
        DB::table('connection_logs')->where('sending_server',$this->id)->delete();
    ...
    public function install_ee($server_ip)
    {
        $server_ip = $server_ip;
        ...
        $this->logger()->info('Creating EE Proxy' . json_encode($response->json()));
    ...
    public static function createMultiFromArray($params)
    {
        $server = new self();
        $server->fill($params);
        ...
        $server->status = self::STATUS_PROCESSING;

        $server->domain = $params['domain'];

        // default name
        if (!$server->name) {
            $server->name = $params['domain'];
        }
        ...
        if (!$server->quota_value) {
            ...
            $options = ['sending_limit' => 'current'];
            $server->options = json_encode($options);
        }

        $server->domain_created_attached = 0;
        $server->multi_server = 1;
        $server->multi_server_linked_with = 0;

        $server->logger()->info('Setting up before creating VPS ' . $server->uid);
    ...
    public function multi_servers()
    {
        return SendingServer::where(['multi_server_linked_with' => $this->id])->get();
    }
    ...
    public function remove_droplet($options, $identity)
    {
        $sending_identities = json_decode($options);
        if (isset($sending_identities->identities->{$identity}->droplet_id)) {
            $droplet = $sending_identities->identities->{$identity}->droplet_id;
            $response =  Http::withHeaders([
                'Authorization' => 'Bearer ' . env('DO_KEY'),
                'content-type' => 'application/json'
            ])->delete(env('DO_DROPLET') . "/droplets/$droplet");
    ...
    {
        $path = storage_path(join_paths('logs', php_sapi_name(), '/sendingserver-' . $this->uid . '.log'));
        return $path;
    }
}
