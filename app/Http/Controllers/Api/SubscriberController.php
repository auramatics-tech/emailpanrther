...
class SubscriberController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (isset($request->campaign_id) && $request->campaign_id) {
            $campaign = \Acelle\Model\Campaign::findByUid($request->campaign_id);
        }

        $subscribers = [];
        foreach ($items as $item) {
            if (isset($item->trackingLog($campaign)->id)) {
                $row = [
                    'uid' => $item->uid,
                    'email' => $item->email,
                    'status' => $item->status,
                ...
                $row['delivered'] = $item->SenttrackingLog($campaign);
                $row['opened'] = $item->openLogs($campaign);
                $row['clicked'] = $item->clickLogs($campaign);
                $tracking_updated_at = $item->trackingLogsCampaign($campaign)->pluck('updated_at')->sortByDesc('updated_at')->first();
                $open_updated_at = isset($item->lastOpenLog($campaign)->updated_at) ? $item->lastOpenLog($campaign)->updated_at : '';
                $clicked_updated_at = isset($item->lastClickLog($campaign)->updated_at) ? $item->lastClickLog($campaign)->updated_at : '';
                if (strtotime($tracking_updated_at) > strtotime($open_updated_at)) {
                    // echo "hre";
                    $row['updated_at'] = $tracking_updated_at;
                }
                if (strtotime($open_updated_at) > strtotime($tracking_updated_at)) {
                    // echo "hre1";
                    $row['updated_at'] = $open_updated_at;
                }
                if (strtotime($clicked_updated_at) > strtotime($open_updated_at)) {
                    // echo "hre2";
                    $row['updated_at'] = $clicked_updated_at;
                }
                $subscribers[] = $row;
            }
        }
    ...
    public function show($uid)
    {
        ...
        $item = \Acelle\Model\Subscriber::where('uid', '=', $uid)
            ->first();
