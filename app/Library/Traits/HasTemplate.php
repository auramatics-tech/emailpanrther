...
trait HasTemplate
{
    ...
    {
        if($this->server_type == 'multi-smtp'){
            $msgId = StringHelper::generateMessageId($this->from_email);
        }else{
        ...
        if ($this->isStdClassSubscriber($subscriber)) {
            ...
            $unsubscribeUrl = $subscriber->generateUnsubscribeUrl($msgId,true,$this->uid);
        ...
        $headers = array(
            'X-Camp-ID' => $this->uid,
            'X-Sub-ID' => $subscriber->uid,
            'X-Cust-ID' => $this->customer->uid,
            'X-Msg-ID' => $msgId,
            'X-SendServ-ID' => $server->uid,
            'Precedence' => 'bulk',
        );
    ...
    public function prepareEmail($subscriber, $server = null, $fromCache = false, $expiresInSeconds = 600)
    {
        ...
        if(isset($server->default_from_email) && $server->default_from_email)
            $message->setFrom(array($server->default_from_email => $this->from_name));
        else
    ...
    public function makeTrackingPixel($msgId)
    {
        ...
        return '<img alt="" src="'.$url.'" width="0" height="0" alt="" style="visibility:hidden" />';
    }
