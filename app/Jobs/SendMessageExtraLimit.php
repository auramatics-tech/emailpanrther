...
class SendMessageExtraLimit implements ShouldQueue
{
    use Batchable;
    use Dispatchable;
     ...
     * @return void
     */
    public function __construct($campaign, Subscriber $subscriber, $server, $triggerId = null, $step = NULL, $variant = NULL)
    {
    ...
    public function send($exceptionCallback = null)
    {
        ...
        try {
            ...
            $sent = $this->server->send($message, [], $msgId);

            // Log successful shot
            // Log::info('sm' . $this->step->id);
            $this->campaign->trackMessage($sent, $this->subscriber, $this->server, $msgId, $this->triggerId, $this->step, $this->variant);
            ...
            if ($this->stopOnError) {
                ...
                $this->campaign->trackMessage(['status' => 'failed', 'error' => $ex->getMessage()], $this->subscriber, $this->server, $msgId, $this->triggerId, $this->step, $this->variant);
            }
        }
    }
}
