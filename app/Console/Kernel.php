...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule)
    {
        ...
        $schedule->command('queue:work --queue=default,batch --timeout=120 --tries=1 --max-time=1800')->everyMinute();
        ...
        // attach:domain
        // $schedule->command('attach:domain')->everyMinute()->withoutOverlapping(60);

        // Sender verifying
        // $schedule->command('verify:tracking')->everyMinute()->withoutOverlapping(60);

        
        $schedule->command('link:server')->everyMinute()->withoutOverlapping(60);
        $schedule->command('check:connection')->hourly();
        $schedule->command('feed:dashboard')->hourly();
        $schedule->command('domain:assignment')->everyMinute()->withoutOverlapping(60);
        // $schedule->command('domainmx:assignment')->everyMinute()->withoutOverlapping(60);

        
        $schedule->command('campaign:speed')->hourly()->timezone('UTC');
        
        $schedule->command('campaign:error')->everyTenMinutes()->timezone('UTC');
    }
